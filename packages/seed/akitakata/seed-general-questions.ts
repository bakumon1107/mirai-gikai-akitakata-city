  ...
  "topics": [
    {
      ...
      "raw_question": "このテーマに対応する議員の発言（質問・提案・意見・引用を含む）を原文のまま抜粋（複数回の発言は改行で連結、800文字以内）",
      "raw_answer": "このテーマに対応する答弁者の発言を原文のまま抜粋（複数回の発言は改行で連結、800文字以内）"
    }
  ]
}
...
- raw_question は質問文だけでなく提案・意見・引用など議員の発言全体を忠実に抜粋すること（800文字以内）
- raw_answer は要約せず、会議録テキストから該当発言を忠実に抜粋すること（800文字以内）
- 著名人・政治家・団体への言及も「議会で事実として発言された内容」であるため、省略せず raw_question に含めること`;
}
