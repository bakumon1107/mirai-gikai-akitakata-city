...
#### 委員会の割り当て目安
登録時点では付託先が公表されていないので、以下は**あくまで仮置き**。
**第1日目の議事録（議会運営委員長の報告＝付託表）が出たら必ずそれで確定させる**（後述「議事録の反映」）。

- 給与・組織・火災予防・議員報酬・行政手続き・教育施設工事 → 総務文教
- 福祉・産業・財産区・施設・下水道・火入れ・国保・体育施設 → 産業厚生
- **市民部所管の条例（印鑑登録・手数料など窓口手続き）→ 産業厚生**
  （令和8年第3回で「行政手続き」とみて総務文教に置いたら、実際は産業厚生への付託だった）
- 補正予算・本予算 → 予算決算
...
市議会HPに各日の議事録（速報版PDF）が出たら、議案のステータスに反映する。
参考実装: `ingest-r8-3-minutes-day1.ts`（共通処理は `lib/bill-pipeline.ts` の `applyBillStatusUpdates`）。

| 日 | 主な内容 | 反映先 |
|---|---|---|
| 第1日目 | 提案理由説明・質疑・**付託**、付託省略案件は即日採決 | `status` / `status_note` / **付託先の確定** |
| 一般質問の日 | 一般質問 | 別スキル `general-questions` |
| 最終日 | 委員長報告・討論・**採決** | `status`（可決・否決）/ `status_note` |

### 読み取り方
- 付託先は**議会運営委員長の報告**に一覧で出る（「議案第50号及び51号の2件は産業厚生常任委員会へ」等）。
  登録時の仮置きと違っていれば `committeeId` で正す。
- 採決は議長の「起立多数であります。よって、本案は原案のとおり可決されました」等で判定。
- 質疑が「（質疑なし）」なら `bill_discussions` には何も登録しない。
  質疑がある場合は `ingest-r8-2-gijiroku.ts` の形式（質問者・答弁者・要旨・原文）で登録する。

### status と status_note の対応
| 議事録の記載 | `status` | `status_note` の例 |
|---|---|---|
| 〇〇常任委員会に付託 | `in_committee` | `9/7 予算決算常任委員会に付託` |
| 起立多数・全員で可決 | `approved` | `9/7 本会議で起立多数により可決（委員会付託省略）` |
| 異議なく同意（同意・諮問） | `approved` | `9/7 本会議で異議なく同意` |
| 否決 | `rejected` | `9/29 本会議で否決` |

**`status_note` は必ず一緒に更新する。** 詳細ページではステータス表示の下に出るため、
放置すると「委員会審査中」の下に登録時の「本会議で上程」が残って矛盾する
（令和8年第2回はこれが放置されている）。

### 注意
- 速報版の議事録に含まれる**個人名（人事案件の候補者など）を解説に書き足さない**。
  反映するのはステータスのみ。
- 追加議案は提出日より前の議事日程に載らないので、日ごとに対象外の議案を確認する。
